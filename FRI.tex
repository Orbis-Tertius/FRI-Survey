...
\]
instead. 
Likewise, the first verifier challenge $\lambda$ can be sampled $N_\lambda$ times, yielding a lowered soundness error of the first round,
\[
\varepsilon_1 = \left(L^+\cdot\frac{C}{|F|}\right)^{N_\lambda}.
\]
Note that this increases only the number of segment polynomials of $h_\lambda(X)$ in the batch for FRI (by the factor $N_\lambda$).
