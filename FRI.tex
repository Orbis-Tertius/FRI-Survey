...
Let us inspect the consequences of the enforced correlated agreement for such a “good” first and second message $(f_1,\ldots,f_w)$ and $(h_{\lambda,0},\ldots,h_{\lambda,d-1})$ from a  ``good'' $z$.
Each function in these messages has $\alpha^+$-agreement with a polynomial of degree at most $|H|-1+2 = k^+ - 1$, and therefore it is $(1-\alpha^+)$-close to $\RS_{k^+}[F,D]$. 
(For now, we ignore the evaluation property of these polynomials.)
%
By the Guruswami-Sudan list decoding bound (see Appendix \ref{s:ListDecoding}), there can exist at most $ L^+$  many elements in
$F[X]^{w+d}$, with each component polynomial of degree  $\leq k^+-1$, and which have correlated agreement with $(f_1$, \ldots, $f_w$, $h_{\lambda,0}, \ldots, h_{\lambda, d-1})$ on some set of density $\geq \alpha^+$.
We shall call such vector of polynomials a \textit{$(w+d)$-configuration}.
%
...
Since there exist more than $L^+\cdot \left(d\cdot (k^+ - 1) + (k - 1)\right)$ many ``good'' $z$, and each establishes a $(w+d)$-configuration which moreover evaluates to the claimed values, we conclude from the pigeon-hole principle that there is at least one $(w+d)$-configuration, 
\[
...
By the degree of the identity, this configuration is a solution of it, and $(p_1,\ldots, p_w)\in F[X]^w$ satisfies
\begin{equation}
...
We have seen that for each ``good'' $\lambda$ there exists a \textit{$w$-configuration}, i.e. an element of $F[X]^w$ with each component polynomial of degree $\leq k^+-1$, sharing correlated agreement with $(f_1,\ldots, f_w)$ on a set of density $\geq \alpha^+$, and moreover which  is a solution of \eqref{e:OverallIdentityAIRmodH}.
Again, by the Guruswami-Sudan list size bound, there can be at most $L^+$ such $w$-configurations.
Since there are at least $L^+\cdot C$ many ``good'' $\lambda$, we conclude again from the pigeon-hole principle that there is at least one $w$-configuration, which we again denote by $(p_1,\ldots, p_w)$, for which there are at least $C$ many “good” $\lambda$ for which \eqref{e:OverallIdentityAIRmodH} holds.
