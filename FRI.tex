...
We have seen that for each ``good'' $\lambda$ there exists a \textit{$w$-configuration}, i.e. an element of $F[X]^w$ with each component polynomial being of degree $\leq k^+-1$ and having $\alpha^+$-correlated agreement with $(f_1,\ldots, f_w)$, which  is a solution of \eqref{e:OverallIdentityAIRmodH}.
