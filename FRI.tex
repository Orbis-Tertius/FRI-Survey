...
In the course the latter we clarify some minor gaps of \cite{ethSTARK}, which are the usage of degree correction factors (these are not needed when applying the DEEP method to a batch of polynomials of the same maximum degree), and the quadratic occurence of the decoder list size bound in their soundness error formula, which can be replaced by a linear term.   
...
An \textit{algebraic intermediate representation} (\textit{AIR}, see \cite{Starks, DEEPFRI} or \cite{ethSTARK}) is defined over an FFT domain $H\subset F$ with generator $g$. 
Each $x$ in $H$ carries a “row” of $w$ witnesses (or, “columns”)
\[
(g_1(x),.., g_w(x)).
\]
For simplicity we assume the constraints are algebraic conditions 
\[
P_0, \ldots ,P_{C-1}\in F[X, X_1,...,X_w,Y_1,...,Y_w]  
\]
between neighboring rows only, where for each $P_i$ there is a succinct evaluation formula $P_i' \in  F(X)[X_1,\ldots,X_w, Y_1,\ldots, Y_w]$ which holds for $X=z$ outside the domain $H$.
In this case satisfiability of the AIR is given by 
\begin{equation}
\label{e:AIRConstraint}
P_i(x, g_1(x),\ldots ,g_w(x),g_1(g\cdot x), \ldots,g_w(g\cdot x)) = 0,
\end{equation}
for all $x$ in $H$ and $i=0,\ldots,C-1$, or in terms of low-degree extensions $p_1(X),\ldots, p_w(X)$,  
\begin{equation}
\label{e:AIRConstraintIdentity}
\begin{aligned}
P_i\big(X, p_1(X), \ldots, p_w(X), p_1(g\cdot X), \ldots, p_1&(g\cdot X)\big) 
\\
&= 0 \bmod v_H(X),
\end{aligned}
\end{equation}
where $v_H(X)$ is the vanishing polynomial of $H$.
Without loss in generality, we may assume that for each constraint the degree in $X$ is bounded by $|H|-1$. 
The \textit{overall degree} of the AIR is 
\[
d = \max_i \deg_{F(X)}(P_i),
\]
where $\deg_{F(X)}(P_i)$ is the total degree of $P_i$ regarded as polynomial over $F(X)$ in the variables $X_1, \ldots, X_w$, $Y_1, \ldots, Y_w$ . 

\begin{rem}
We point out a slight difference between our definition of an AIR and those from \cite{DEEPFRI} and \cite{ethSTARK}.
We directly consider periodic and boundary constraints as polynomial identities over the entire domain $H$:
%, as used in the definition of an AIR from \cite{DEEPFRI} and \cite{ethSTARK} are captured by our our definition. 
If $a_i\cdot H_i$ is the coset of a subgroup $H_i$ of $H$, on which we want to enforce a polynomial relation between two neighboring rows, then the polynomial
\[
s(X) =\frac{v_H(X)}{v_{a_i\cdot H_i}(X)}  \in F[X]
\]
can be used to ``select'' enforcement of the relation on $a_i\cdot H_i$ only. 
For this reason we assume each $P_i$  as elements of  $F[X, X_1,\ldots, X_w, Y_1,\ldots, Y_w]$ thinking selector polynomials being integrated into $P_i$. 
Notice that, although $\deg s_i(X) = O(|H|)$, the polynomial $s(X)$ can be succinctly evaluated outside $H$ using the above rational representation, since 
\[
...
k = |H|.
\end{equation}
The agreement parameter used for FRI is chosen slightly larger than $\alpha = \left(1+\frac{1}{2m}\right)\cdot\sqrt\rho$, $m\geq 3$, namely 
\begin{equation}
...
Their denominators can be quadratic and hence the degree of the non-quotients by $\leq |H|-1 + 2$.
The low-degree extensions $p_i(X)\in F[X]$ of the witness functions $g_i$ on $H$ are provided as code words over $D$, and to use again the same code for a polynomial $h(X)$ of larger degree, we split it into segment polynomials of degree $|H|-1$:
\begin{equation}
\label{e:Splitting}
h(X)=h_0(X^d)+X\cdot h_1(X^d)+ \ldots + X^{d-1} \cdot h_{d-1}(X^d),
\end{equation}
where $\deg h_j(X)\leq |H|-1$, $j=0,..,d-1$.

The DEEP-ALI protocol (for simplicity without zero-knowledge) for our AIR is as follows: 

\begin{protocol}
Let $p_1(X), \ldots, p_w(X)\in F[X]$ be polynomials of degree $\deg p_i(X)\leq |H|-1$ satisfying the AIR constraints \eqref{e:AIRConstraint} for all $x\in H$ and $i=1,\ldots, C$.
\begin{enumerate}
\item
...
\begin{equation*}
\sum_{i=0}^{C-1} \lambda^i\cdot P_i(X, p_1(X),\ldots ,p_w(X),p_1(gX), \ldots, p_w(gX)) = h_\lambda(X)\cdot v_H(X),
\end{equation*}
splits it into its segments $h_{\lambda, 0}(X), \ldots , h_{\lambda, d - 1}(X)$ as in \eqref{e:Splitting}, and sends their domain evaluation oracles $[h_{\lambda,0}]$, \ldots, $[h_{\lambda,d - 1}]$ to the verifier.  
The overall identity to be proven is 
\begin{equation}
\label{e:OverallIdentityAIR}
\begin{aligned}
\sum_{i=0}^{C-1} \lambda^i\cdot P_i(X, p_1(X), \ldots, p_w(X), p_1&(gX), \ldots, p_w(gX))
\\ 
&= v_H(X)\cdot \sum_{j=0}^{d-1} X^j\cdot  h_{\lambda,j}(X^d).
\end{aligned}
\end{equation}
...
\varepsilon < L^+ \cdot \left(\frac{C}{|F|} + \frac{d\cdot (k^+ - 1) + (k - 1)}{|F| - |D\cup H|}\right) + \varepsilon_{FRI},
\end{equation}
...
This is due to the application of two separate FRI arguments, one batched for the witness polynomials, and another one for the overall quotient polynomial.  
...
\begin{rem}
As pointed out in the introduction, the present soundness analysis improves over \cite{ethSTARK}, Theorem 4. 
There, the list bound $L^+$ occurs also quadratic, which 
\end{rem}

\begin{proof}
We denote $\varepsilon_1=L^+ \cdot \frac{C}{|F|}$, $\varepsilon_2 = L^+ \cdot \frac{d\cdot k^+}{|F|}$ , and $\varepsilon_3=\varepsilon_{FRI}$. 
Suppose that $P^*$ is an adversary which succeeds the verifier with probability $\varepsilon_1+\varepsilon_2+\varepsilon_3$ .
Then there exists a first message of $P^*$, i.e. words  $f_1, \ldots, f_w$  on $D$, on which $P^*$ succeeds with probability $\geq\varepsilon$ , and hence
\[
...
\]
For each such “good” $z\in F\setminus D$ (by the definition of $\varepsilon_2$, there are more than $L^+\cdot d\cdot( k^+ - 1)$ many) the soundness of FRI enforces the quotients
\[ 
\left( 
...
%
Since there are  more than $L^+\cdot d\cdot (k^+ - 1)$ many good $z$, and each such good $z$ establishes a ``$j$-configuration'' which evaluates to the claimed values, we conclude from the pigeon-hole principle that there is at least one ``$j$-configuration'' 
\[
(p_1,\ldots, p_w, h_{\lambda, 0}, \ldots, h_{\lambda,d-1}) = \left(p_1^{(j)},\ldots, p_w^{(j)},h_{\lambda,0}^{(j)}, \ldots , h_{\lambda, d-1}^{(j)} \right)
\] 
for which the overall identity \eqref{e:OverallIdentityAIR} holds for $\lambda$ at more than $d\cdot (k^+-1)$ many $z$. 
By the degree of the identity, this configuration is a solution of it, and therefore
\begin{equation}
\label{e:OverallIdentityAIRmodH}
\sum_{i=0}^{C-1} \lambda^i\cdot P_i(X, p_1(X), \ldots, p_w(X), p_1(gX), \ldots, p_w(g X)) 
= 0 \bmod v_H(X)
\end{equation}
...
\[
 P_i(X, p_1(X), \ldots, p_w(X), p_1(gX), \ldots, p_w(g X)) = 0 \bmod v_H(X)
\]
for every $i =0, \ldots, C-1$, which shows that the values of $(p_1,\ldots, p_w)$ over $H$ satisfy the constraints the AIR.
...
We only provide a brief sketch how to build the extractor in the oracle model, given a prover $P^*$ which succeeds with a probability of at least the above soundness error bound:
\begin{enumerate}
\item
Apply the forking lemma to sample a “good” first message $[f_1],\ldots , [f_w]$ on which the prover succeeds with a probability greater than the soundness error from Theorem \ref{thm:DEEPsoundness}.
