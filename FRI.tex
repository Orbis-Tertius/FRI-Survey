...
%
By the Guruswami-Sudan list size bound\footnotemark (see Appendix \ref{s:ListDecoding}) there are at most $ L^+$  such $\alpha^+$-configurations.
\footnotetext{%
By correlated agreement each of the component functions is $(1-\alpha^+)$-close to $\RS_{k^+}[F,D]$.
}
...
Instead of drawing the DEEP query from an extension field, one may also take several DEEP queries $z_1,\ldots , z_{N_z}$ from $F\setminus D$, and apply FRI to the batch of all resulting quotients. 
...
\[
\left(L^+\cdot\frac{d\cdot (k^+ - 1) + (k - 1)}{|F\setminus (D\cup H)|}\right)^{N_z}
\]
instead. 
Similarly, the first verifier challenge $\lambda$ can be sampled $N_\lambda$ times, to yield a lowered soundness error of the first round,
\[
\left(L^+\cdot\frac{C}{|F|}\right)^{N_\lambda}.
\]
Again, this comes at the cost of increasing the number of polynomials in the batch for FRI by the factor $N_\lambda$, which can be still acceptable.
However, the resampling of FRI challenges would increase the proof size too much. 
Hence at least for  FRI, extension field sampling is preferable.  
